...
| **Per-user quota endpoint** | `PATCH /api/v1/admin/quotas/{username}` — admin sets per-user token quota. Requires new `user_quotas` DB table. | Phase 2 | Medium |
| **Bulk usage endpoint** | `GET /api/v1/admin/usage?range=mtd` — admin scrapes all users' token usage with cost. Aggregates from existing metering data. New M2M endpoint (offline token auth, not session cookie). | Phase 2 | Medium |
| **Self-service usage** | `GET /v1/usage/me` — user views own token consumption. Auth via API key → username resolution (already exists in validation path). | Phase 2 | Small |
| **Self-service quota** | `GET /v1/quota/me` — user views remaining budget. Same auth path as above. | Phase 2 | Small |
...
| Gap | Description | Phase | Effort |
|-----|------------|-------|--------|
| **Shared DB setup** | All clusters must connect to the same PostgreSQL instance. Connection string in maas-api and metering-service configs. Requires network connectivity (VPC peering, private endpoints) between clusters and DB. | Phase 1 | Medium |
...
- maas-api: admin quota + usage endpoints
- maas-api: self-service usage/quota endpoints
